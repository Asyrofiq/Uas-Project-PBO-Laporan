...
class snake():
    def __init__(self,speed, size):
        self.pos = [20,20]
        self.image = pg.Surface((12*size,12*size))
...
class game():
    ...
    def over(self):
        while 1:
            ...
            txts = pg.font.SysFont('Times New Roman',50).render('Your Score  :',True,(255,255,255))
            txtrect = txts.get_rect()
            txtrect.topleft =(20,150)
            self.screen.blit(txts,txtrect)
            txts = pg.font.SysFont('Times New Roman',50).render(str(self.snake.score),True,(255,255,255))
            txtrect = txts.get_rect()
...
class startmenu():
    def __init__(self):
        self.screen = pg.display.set_mode((800,450))
        self.b1 = '(340, 300,100,50),"Play Game", [(255,0,0), (0,150,0)], action = self.start'
    ...
    def make_text(self, x, y, text, size=20, color = (0,0,0), a = False):
        txts = pg.font.SysFont('Times New Roman', size).render(text, True, color)
        txtrect = txts.get_rect()
        txtrect.topleft = (x,y)
        if a == True:
            txtrect.center = (x,y)
    ...
    def make_button(self, pos, text, color, action=None, textsize=20):
        ...
        circle = pg.Surface([min(rect.size)*3]*2,pg.SRCALPHA)
        pg.draw.ellipse(circle,(255,0,0),circle.get_rect(),0)
        circle  = pg.transform.smoothscale(circle,[int(min(rect.size)*0.5)]*2)
    ...
    def start(self):
        self.b1 = '(340, 300,100,50),"Start", [(255,0,0), (0,150,0)], action = self.n'
        self.buttons = [self.b1]
